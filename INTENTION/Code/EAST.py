...
Canal15_raw = np.zeros(Canalacq)
Canal16_raw = np.zeros(Canalacq)
...
emgArray15 = np.zeros(arraysize)
emgArray16 = np.zeros(arraysize)
...
#                Canal14_Raw_Total=np.concatenate((Canal14_Raw_Total,Canal14_raw[c]),axis=None)
                ...
                Canal15_raw[c] = (ConvFact*(int.from_bytes(info[k],byteorder='little', signed=True)) - Offset[1]) 
#                Canal15_Raw_Total=np.concatenate((Canal15_Raw_Total,Canal15_raw[c]),axis=None)
                
            elif j == c16:
                Canal16_raw[c] = (ConvFact*(int.from_bytes(info[k],byteorder='little', signed=True)) - Offset[1]) 
#                Canal16_Raw_Total=np.concatenate((Canal16_Raw_Total,Canal16_raw[c]),axis=None)                
    
            k=k+1 
        c=c+1
        
#    Channel_raw = [Canal1_raw, Canal2_raw, Canal3_raw, Canal4_raw, Canal5_raw,
#                   Canal6_raw, Canal7_raw, Canal8_raw, Canal9_raw, Canal10_raw,
#                   Canal11_raw, Canal12_raw]
    
#    return Channel_raw
        


def envelope(Canal1_raw,Canal2_raw,Canal3_raw,Canal4_raw,Canal13_raw,Canal14_raw,Canal15_raw,Canal16_raw):
    
    emgArray1[0:arraychsize] = emgArray1[chansize:arraysize]
    emgArray1[arraychsize:arraysize] = Canal1_raw   
    emgAbs=np.abs(emgArray1)
#    Canal1_noise = signal.filtfilt(e,f,emgAbs) 
...
#    Canal2_noise = signal.filtfilt(e,f,emgAbs)
...
#    Canal3_noise = signal.filtfilt(e,f,emgAbs) 
...
#    Canal4_noise = signal.filtfilt(e,f,emgAbs) 
#    Canal4_env = signal.filtfilt(b,a,Canal4_noise)  
    Canal4_env = signal.filtfilt(b,a,emgAbs) 
    Canal4 = Canal4_env[arraychsize:arraysize]
    
    
    emgArray13[0:arraychsize] = emgArray13[chansize:arraysize]
    emgArray13[arraychsize:arraysize] = Canal13_raw
    emgAbs=np.abs(emgArray13)
#    Canal13_noise = signal.filtfilt(e,f,emgAbs) 
#    Canal13_env=signal.filtfilt(b,a,Canal13_noise)   
    Canal13_env=signal.filtfilt(b,a,emgAbs) 
    Canal13 = Canal13_env[arraychsize:arraysize]
    
    
    emgArray14[0:arraychsize] = emgArray14[chansize:arraysize]
    emgArray14[arraychsize:arraysize] = Canal14_raw
    emgAbs=np.abs(emgArray14)
#    Canal14_noise = signal.filtfilt(e,f,emgAbs) 
#    Canal14_env=signal.filtfilt(b,a,Canal14_noise)  
    Canal14_env=signal.filtfilt(b,a,emgAbs) 
    Canal14 = Canal14_env[arraychsize:arraysize]
    
    emgArray15[0:arraychsize] = emgArray15[chansize:arraysize]
    emgArray15[arraychsize:arraysize] = Canal15_raw
    emgAbs=np.abs(emgArray15)
#    Canal15_noise = signal.filtfilt(e,f,emgAbs) 
#    Canal15_env=signal.filtfilt(b,a,Canal15_noise)   
    Canal15_env=signal.filtfilt(b,a,emgAbs) 
    Canal15 = Canal15_env[arraychsize:arraysize]
    
    
    emgArray16[0:arraychsize] = emgArray16[chansize:arraysize]
    emgArray16[arraychsize:arraysize] = Canal16_raw
    emgAbs=np.abs(emgArray16)
#    Canal16_noise = signal.filtfilt(e,f,emgAbs) 
#    Canal16_env=signal.filtfilt(b,a,Canal16_noise)  
    Canal16_env=signal.filtfilt(b,a,emgAbs) 
    Canal16 = Canal16_env[arraychsize:arraysize]
    
    
    Channel = [Canal1, Canal2, Canal3, Canal4, Canal13, Canal14, Canal15, Canal16]
...
#        while sfprocess._value:
    ...
    #        for h in range(0,repeat*10):
                ...
                try:
                    ...
                    Canal3_Raw_Total = np.concatenate((Canal3_Raw_Total,Canal3_raw),axis=None)
                    Canal4_Raw_Total = np.concatenate((Canal4_Raw_Total,Canal4_raw),axis=None)
                    Canal5_Raw_Total = np.concatenate((Canal5_Raw_Total,Canal5_raw),axis=None)
                    Canal6_Raw_Total = np.concatenate((Canal6_Raw_Total,Canal6_raw),axis=None)
                    Canal7_Raw_Total = np.concatenate((Canal7_Raw_Total,Canal7_raw),axis=None)
                    Canal8_Raw_Total = np.concatenate((Canal8_Raw_Total,Canal8_raw),axis=None)
                    Canal9_Raw_Total = np.concatenate((Canal9_Raw_Total,Canal9_raw),axis=None)
                    Canal10_Raw_Total = np.concatenate((Canal10_Raw_Total,Canal10_raw),axis=None)
                    Canal11_Raw_Total = np.concatenate((Canal11_Raw_Total,Canal11_raw),axis=None)
                    Canal12_Raw_Total = np.concatenate((Canal12_Raw_Total,Canal12_raw),axis=None)
                    
                    Canal13_Raw_Total = np.concatenate((Canal13_Raw_Total,Canal13_raw),axis=None)
                    Canal14_Raw_Total = np.concatenate((Canal14_Raw_Total,Canal14_raw),axis=None)
                    Canal15_Raw_Total = np.concatenate((Canal15_Raw_Total,Canal15_raw),axis=None)
                    Canal16_Raw_Total = np.concatenate((Canal16_Raw_Total,Canal16_raw),axis=None)    
                    ...
                    if not sfRead._value:
                        ...
                        sfRead.release()#read torque
                    
                    #envelope
                            #falta bloquear el acceso a Canal1_raw
                    Channel = envelope(Canal1_raw,Canal2_raw,Canal3_raw,Canal4_raw,Canal13_raw,Canal14_raw,Canal15_raw,Canal16_raw)
                            
                    Canal1_Env_Total = np.concatenate((Canal1_Env_Total,Channel[0]),axis=None)
                    Canal2_Env_Total = np.concatenate((Canal2_Env_Total,Channel[1]),axis=None)
                    Canal3_Env_Total = np.concatenate((Canal3_Env_Total,Channel[2]),axis=None)
                    Canal4_Env_Total = np.concatenate((Canal4_Env_Total,Channel[3]),axis=None)
                    Canal13_Env_Total = np.concatenate((Canal13_Env_Total,Channel[4]),axis=None)
                    Canal14_Env_Total = np.concatenate((Canal14_Env_Total,Channel[5]),axis=None)
                    Canal15_Env_Total = np.concatenate((Canal15_Env_Total,Channel[6]),axis=None)
                    Canal16_Env_Total = np.concatenate((Canal16_Env_Total,Channel[7]),axis=None)  
        ...
        emg = [Canal1_Raw_Total, Canal2_Raw_Total, Canal3_Raw_Total, Canal4_Raw_Total,
               ...
               Canal13_Raw_Total, Canal14_Raw_Total, Canal15_Raw_Total, Canal16_Raw_Total,
               Canal1_Env_Total, Canal2_Env_Total, Canal3_Env_Total, Canal4_Env_Total,
               Canal13_Env_Total, Canal14_Env_Total, Canal15_Env_Total, Canal16_Env_Total, tiempo]
...
def OnsetGetEMG(features, max_minFeatures, intra, sfMove, sfSend, sfprocess, sfRead, sfexit, repeat):
...
#                        print(detectionOn)
                        ...
                        if (detectionOn[1] or detectionOn[2]) and fstep:
                            offset =1
                            #MUEVE PIERNA DERECHA
                            #TimeStamp
#                            detectTime=datetime.now().time()
#                            print("detect")
#                            print("min: ",detectTime.minute, " sec: ", detectTime.second, " mls: ",detectTime.microsecond)
                            
                            sfMove.release()
                            if detectionOn[1] and detectionOn[2]:
                                Onset_Total=np.concatenate((Onset_Total,5),axis=None)
                            elif detectionOn[1]:
                                Onset_Total=np.concatenate((Onset_Total,2),axis=None)
                            else:
                                Onset_Total=np.concatenate((Onset_Total,3),axis=None)
                            fstep = 0
                            print("detected FS")
                            
                        #offset
                        elif sfSend._value and offset:
                            
                            Onset_Total=np.concatenate((Onset_Total,7),axis=None)
                            offset=0
                        
                        #multiple muscles Rf or Sol_antagonista(izq)    
                        elif (detectionOn[1] or detectionOn[2]) and sfSend._value and stepsize:
                            offset =1
                            #MUEVE PIERNA DERECHA
                            #TimeStamp
#                            detectTime=datetime.now().time()
...
#                            print("min: ",detectTime.minute, " sec: ", detectTime.second, " mls: ",detectTime.microsecond)
                            ...
                            sfMove.release()#exo move
                            stepsize = 0
                            print("(detect) MUEVE PIERNA DERECHA")
...
#                            Onset_Total=np.concatenate((Onset_Total,0),axis=None)
                        ...
                        elif (detectionOn[3] or detectionOn[0]) and sfSend._value and not stepsize:
                            offset =1
...
#                            print("min: ",detectTime.minute, " sec: ", detectTime.second, " mls: ",detectTime.microsecond)
                            ...
                            sfMove.release()#exo move
                            stepsize = 1
                            print("(detect) MUEVE PIERNA IZQUIERDA")
                            
                            if detectionOn[3] and detectionOn[0]:
                                Onset_Total=np.concatenate((Onset_Total,-5),axis=None)
                            elif detectionOn[0]:
                                Onset_Total=np.concatenate((Onset_Total,1),axis=None)
                            else:
                                Onset_Total=np.concatenate((Onset_Total,4),axis=None)
                            sfSend.acquire()
        ...
        emg = [Canal1_Raw_Total, Canal2_Raw_Total, Canal3_Raw_Total, Canal4_Raw_Total,
               ...
               Canal13_Raw_Total, Canal14_Raw_Total, Canal15_Raw_Total, Canal16_Raw_Total,
               Canal1_Env_Total, Canal2_Env_Total, Canal3_Env_Total, Canal4_Env_Total,
               Canal13_Env_Total, Canal14_Env_Total, Canal15_Env_Total, Canal16_Env_Total, 
               tiempo]
